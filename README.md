...
Phase 1〜7のlocal実装は完了・push・tag済みです。Phase 8 local smokeは89チェックpass、manual UI rehearsal follow-upも完了・push済みです。Phase 9ではQuiz World専用Supabase Preview projectとVercel Preview projectを作成し、Preview DB migration / seed、DB smoke、Vercel Preview env設定、Git連携Preview deployまで完了しています。Step G Preview smokeは、Framework PresetをNext.jsへ明示した後の新しいGit連携Preview deploymentで入口確認とMVP主要ループ本体をpassしました。Step H cleanup / resetも完了し、Preview DBはseed状態へ戻っています。Phase 9 Preview ready地点は `v0.10.0-phase9-preview-ready` tagで固定済みです。Phase 10 participant guide / admin ops checklist は作成済みです。Phase 10 owner/admin最終確認後の人間決定事項も反映済みで、まず信頼できる1名へ個別DMで限定共有し、問題がなければ2名目へ拡張する方針です。Phase 10の1名限定共有前UI整理はPreview deploymentへ反映済みで、反映後確認もpass済みです。追加で `/home` の参加者向け表示から内部仕様文言を削除し、新着確認処理は内部処理として維持する方針に整理済みです。さらに `/create` と `/world` の参加者向け表示から内部仕様文言、未確定指標、平均評価、解放条件を削除し、`/world` は参加状況確認に絞る方針へ整理済みです。この `/create` / `/world` UI整理もGit連携Preview deploymentへ反映済みで、反映後確認もpass済みです。Phase 10の1名テスト中に見つかった `/profile` P1も整理済みで、通常ユーザーにraw `role` / `status` や保存できないフォームを出さない実装をcommit / pushし、Git連携Preview deploymentへ反映済みです。通常ユーザーのPreview実画面で `/profile` にraw `role` / `status` と保存できないフォームが出ないこと、headerにAdmin導線が出ないこと、`/admin` 直接アクセスが拒否されることを確認済みです。GPT Pro追加レビュー後の2名目共有前小P1として、`/account` のraw `status` 表示と `/admin` 非許可画面の内部説明も整理し、commit / push後にGit連携Preview deploymentへ反映済みです。Preview上で `/account` にraw `status` / `active` が表示されないこと、`/admin` 非許可画面に内部DB条件が表示されないこと、`/api/admin/*` の保護が維持されていることを確認済みです。信頼できる1名への個別共有は開始済みで、Preview URLと参加者別invite codeは個別DMでのみ共有しています。2名目共有前read-only確認と2名目用参加者別invite code発行も完了済みで、2名目への限定共有はGO候補です。ただし、2名目へのPreview URL / invite code共有実行はまだ行っていません。10人テスト候補全員への共有、SNSや公開ページでの共有はまだ行いません。Production envは未設定、Production deployは行っていません。Stripe、Web Push、Realtimeはまだ行っていません。既存Smart Buzzerとは別プロジェクトとして扱います。
...
- [Phase 10 first participant test results](docs/quiz-world/quiz-world-phase-10-first-participant-test-results.md)
- [Phase 10 second participant prep results](docs/quiz-world/quiz-world-phase-10-second-participant-prep-results.md)
...
- Framework Preset明示前の新しいGit連携Preview deploymentでは、通常アクセスはDeployment Protectionにより `/` と `/api/world` が401、Vercel CLI bypassは `/` と `/api/world` が404、Chrome直接表示も `/` が404となりました。この時点では入口条件を満たせず、MVP主要ループは未実行でした。
- Framework Preset明示前のStep G判断はNO-GOでした。Shareable Linkやbypass secretの実値はrepo/docsに書きません。
...
- Build logでNext.js routesが出力され、`/`、`/signup`、`/api/world` などが含まれることを確認しました。secret実値は記録していません。
- Framework Preset明示後のStep G入口確認では、通常アクセスはDeployment Protectionの401、Vercel CLIのprotection bypass経由では `/` がQuiz WorldのHTML、`/api/world` が初期world JSONを返しました。
- Framework Preset明示後のStep G Preview smoke本体では、signup、初期admin role、login、question作成、launch、/home、start_at前後、answer、result、rating、report、rank_events、admin確認、admin_audit_logsまで49項目を確認しpassしました。
...
- Phase 9 Step Hとして、Preview DB cleanup / resetを実行済みです。
- Step Hでは、Preview DBをfull resetし、migration / Preview seedを再投入しました。
...
- Preview seed再投入手順とAuth users cleanup手順に沿って実行し、Auth users追加cleanupは不要でした。
- Phase 9 Step H cleanup / resetを実行済みです。`supabase db reset --linked --no-seed` と `supabase/seed.preview.sql` 適用により、Preview DBはseed状態へ戻りました。
...
- Phase 9 Step H完了時点ではPreview URL共有範囲をowner/adminのみに維持しました。現在はPhase 10で信頼できる1名への個別共有を開始済みです。10人テスト候補全員への共有はまだ行いません。`v0.10.0-phase9-preview-ready` tagは作成・push済みです。
...
- 信頼できる1名への個別共有は開始済みです。Preview URLと参加者別invite codeは個別DMでのみ共有し、実値はdocs/repoに書いていません。2名目への共有、10人テスト候補全員への共有、SNSや公開ページでの共有はまだ行いません。
...
- `/create` / `/world` UI整理はGit連携 / `preview` / `8937775` のPreview deploymentへ反映済みです。deployment statusはReadyで、Preview反映後確認もpass済みです。Production deploymentは追加作成されていません。
- 1名参加後のread-only確認では、`auth.users=2`、`profiles=2`、`world_members=2`、`activeMemberCount=2` を確認しました。内訳はadmin 1名、通常user 1名で想定通りです。参加者別invite codeは使用済み1件、共通Preview invite code `SEASON0-PREVIEW-001` はactiveで予備として維持しています。参加者別invite code実値とemail実値はdocs/repoに書いていません。
- Phase 10の1名テスト中にGPT Proレビューを実施し、`/profile` のP1として、通常ユーザーにraw `role` / `status` を見せないこと、保存できない表示名・通知設定フォームを出さないこと、内部向けの「次Phase」文言を消すことを確認しました。`/profile` はスコア、ランク、最近の履歴、最小限のプロフィール表示に絞り、admin activeユーザーだけに最小の管理者表示を出す方針へ整理済みです。
- 表示名編集、通知設定保存、通知モード、quiet hours、1日の通知上限は後続課題として扱います。Phase 10の1名テストでは、保存できない入力フォームを参加者向けUIに出しません。
- Phase 10は信頼できる1名の個別テストを継続中です。`/account` と `/admin` の小P1整理はPreview反映後確認まで完了済みです。2名目への限定共有はGO候補ですが、共有実行はまだ行っていません。最大10名への拡張はまだ行いません。
- GPT Pro追加レビュー後、2名目共有前のP0/P1実装修正は残っていないことを確認しました。2名目共有前read-only確認では `auth.users=2`、`profiles=2`、`world_members=2`、`activeMemberCount=2`、`remainingSeats=8` を確認しました。2名目用の参加者別invite codeを1件発行済みで、`max_uses=1`、active、`use_count=0`、`admin_audit_logs` の `invite_created` を確認済みです。参加者別invite code実値はdocs/repoに書いていません。2名目へのPreview URL / invite code共有はまだ実行していません。
