...
- `NEXT_PUBLIC_APP_URL` はPreview URL未確定のため未設定です。Step F Preview deploy前後で実値を確定してから設定します。
