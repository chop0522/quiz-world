...
- Build logでNext.js routesが出力され、`/`、`/signup`、`/api/world` などが含まれることを確認済み。secret実値は記録しない
- Step G smoke本体はまだ再実行していない。Production env / Production custom domainは未設定で、追加Production deployは発生していない
