...
- 最終GO/NO-GO判断はStep DでVercel project作成済み。Step D follow-upでGitHub repo接続済み。Step EはProduction Branchを `production-hold` へ変更後にVercel Preview env設定済み。Production envとPreview deployは未実行。
...
- `NEXT_PUBLIC_APP_URL` はPreview URL未確定のため未設定。Step F Preview deploy前後で実値を確定してから設定する
...
- Step F Preview deploy前に、Ignored Build Stepの一時設定 `exit 0` を解除するか、Preview deployを許可する条件付きcommandへ見直す。
- Preview deploy前に `NEXT_PUBLIC_APP_URL` の設定タイミングを確定する。
