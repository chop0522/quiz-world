...
現在はPhase 5のresult / rating / reports計画中である。
Supabase / Vercel / Stripe のcloud環境、production deploy、ランキング本格反映、admin本実装、Realtime、Web Pushはまだ扱わない。
...
| Phase 5評価 | ratingできるのはrecipientのみ。author本人は自分の問題をratingできない。同一launch/raterは1件のみ。 |
...
| Phase 5 rating理由タグ | MVP初期は1タグにするか、複数タグを `jsonb` で持つかを決める。 |
| Phase 5 report重複 | 同一reportの重複をどの単位で防ぐかを決める。 |
| Phase 5 review_required更新 | 同一問題2件以上のreportで `question.status = review_required` を自動更新するか、admin候補表示に留めるかを決める。 |
...
- Phase 5計画範囲外の本格API実装
- Supabase / Vercel / Stripe のcloud環境作成

Phase 5計画段階ではQuiz World専用リポジトリ内のdocs更新のみを扱う。
Phase 5実装へ進む場合も、対象は result、rating、reports に限定する。
...
- `quiz-world-phase-5-result-rating-plan.md` をChatGPT Proに渡してレビューする。
- rating理由タグの保存形式、report重複防止、`review_required` 更新方式を決める。

### B. Phase 5 local実装

- レビュー後、Supabase local前提で result、rating、reports を実装する。
- rank_events本格反映、Web Push、Realtime、admin本実装、cloud環境はまだ作らない。

## 現時点の推奨

Phase 5計画docsをレビューし、未確定の小さな保存方針を決めてからPhase 5 local実装へ進む。
