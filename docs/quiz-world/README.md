...
- 最終GO/NO-GO判断はStep DでVercel project作成済み。Step D follow-upでGitHub repo接続済み。Step EはProduction Branchを `production-hold` へ変更後にVercel Preview env設定済み。Step F Preview deployは実行済みでReady。Production envとProduction deployは未実行。
...
- `NEXT_PUBLIC_APP_URL` はPreview URL未確定のため未設定。現状のapp codeでは参照されておらず、未設定でもbuildは成功済み。必要ならStep G以降で設定する
