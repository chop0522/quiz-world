...
### A. Phase 8 local smoke resultsレビュー

- Phase 8 resultsをレビューする。
- 10人テスト前に残る課題を確認する。

### B. 人間操作での最終UI rehearsal

- ブラウザ自動操作ではなく、人間操作で `/signup` から `/admin` までの最終UI確認を行うか判断する。
...
Phase 1〜7 local実装は完了・push・tag済み。Phase 8 10-user local smoke / ops rehearsalもSupabase localのみで実行済み、89チェックpass、DB reset済み。次はPhase 8 resultsをレビューし、Phase 9 Preview環境検討へ進むか、先に人間操作での最終UI rehearsalを行うか判断する。
