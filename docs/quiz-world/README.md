...
Phase 9 Preview環境計画はcommit・push済みで、実行前チェックリストも作成済みである。Step AとしてQuiz World専用Supabase development projectを作成済み、Step BとしてPreview DBへのmigration / seed適用済み、Step CとしてPreview DB smokeをpass済みである。Step DとしてQuiz World専用Vercel project `quiz-world-preview` を作成済みで、Step D follow-upとしてGitHub repo `chop0522/quiz-world` への接続も完了済みである。Step EとしてVercel Preview envをPreview environmentのみに設定済みである。Step FとしてIgnored Build Stepを `preview` branchだけbuild許可する条件式へ変更し、`preview` branchを作成・pushしてPreview deployを実行済みである。Preview deploymentはReadyである。Step G Preview smokeは一度NO-GOだったが、Vercel Project SettingsでFramework PresetをNext.jsへ明示した後の新しいGit連携Preview deploymentで入口確認とMVP主要ループ本体をpassした。Step HとしてPreview DB cleanup / resetを実行済みである。full reset + migration再適用 + Preview seed再投入により、初期worldとPreview invite code `SEASON0-PREVIEW-001` が復元され、Step G smoke由来データは削除された。Preview URLの軽量確認もpass済みである。Phase 9 Preview ready地点は `v0.10.0-phase9-preview-ready` tagで固定済みである。Phase 10 participant guide / admin ops checklist は作成済みである。Phase 10 owner/admin最終確認後の人間決定事項も反映済みで、まず信頼できる1名へ個別DMで限定共有し、問題がなければ2名目へ拡張する方針である。Phase 10の1名限定共有前UI整理として、ログイン状態に応じたheader/nav表示、`/account`、`/account/password`、logout配置、admin導線の表示条件を最小修正済みである。logoutはheaderに出さず、`/account` に置く。`npm run typecheck` / `npm run lint` / `npm run test` / `npm run build` はpass済みである。このUI整理はGit連携 / `preview` / `b48d73a` のPreview deploymentへ反映済みで、deployment statusはReadyである。Preview反映後確認では、未ログイン時のlogin / signup導線、`/account`、`/account/password`、`/admin` の未ログイン保護、`/api/admin/*` の401、headerにlogoutが出ないことを確認済みである。追加で `/home` の参加者向けUIから内部仕様文言を削除し、15秒ごとの新着確認は内部処理として維持する方針に整理済みである。`/home` UI整理はGit連携 / `preview` / `cdab98c` のPreview deploymentへ反映済みで、deployment statusはReadyである。さらに `/create` と `/world` の参加者向けUIから内部仕様文言と未確定指標を削除済みである。`/create` は四択クイズ作成と一覧からの出題に絞り、`/world` は現在の参加人数、参加枠、残り枠、Seasonの確認に絞る。平均評価や参加枠解放条件は後続課題として扱う。この `/create` / `/world` UI整理はGit連携 / `preview` / `8937775` のPreview deploymentへ反映済みで、deployment statusはReadyであり、Preview反映後確認もpass済みである。Phase 10の1名テスト中に見つかった `/profile` P1も整理済みで、通常ユーザーにraw `role` / `status` や保存できないフォームを出さない実装をcommit / pushし、Git連携 / `preview` / `38b67b5` のPreview deploymentへ反映済みである。deployment statusはReadyで、Production deploymentは追加作成されていない。通常ユーザーのPreview実画面で `/profile` にraw `role` / `status` と保存できないフォームが出ないこと、headerにAdmin導線が出ないこと、`/admin` 直接アクセスが403で拒否されることを確認済みである。信頼できる1名への個別共有は開始済みで、Preview URLと参加者別invite codeは個別DMでのみ共有している。GPT Pro追加レビューでは、2名目へ進む前の小P1として `/account` に残るraw `status` / `active` 表示と、non-admin / 未ログインの `/admin` 非許可画面に残る内部説明を整理する判断になった。`/account` は表示名、password変更、logoutに絞り、`/admin` 非許可画面は一般向け文言にする。forgot passwordはPhase 10の1〜2名テストではP2とし、まずはownerへの個別DMサポートで運用する。username変更はP2、avatar設定はP3として後続課題にする。`/account` と `/admin` の小P1整理はcommit / push済みで、Git連携 / `preview` / `3d0895c` のPreview deploymentへ反映済みである。Preview反映後確認では、`/account` にraw `status` / `active` が表示されないこと、未ログインの `/admin` 非許可画面に内部DB条件が表示されないこと、`/api/admin/users` が未ログイン時に `ログインが必要です。` を返すことを確認済みである。2名目への限定共有はGO候補だが、共有実行はまだ行っていない。10人テスト候補全員への共有、SNSや公開ページでの共有はまだ行っていない。1名参加後のread-only確認では、admin 1名と通常user 1名のactive member 2名を確認している。Production envは未設定、追加Production deployは未実行である。Stripe、Realtime、Web Pushはまだ扱わない。

Phase 10の1名テスト中にGPT Proレビューを実施し、`/profile` のP1として、通常ユーザーにraw `role` / `status` を見せないこと、保存できない表示名・通知設定フォームを出さないこと、内部向けの「次Phase」文言を消すことを確認した。`/profile` はスコア、ランク、最近の履歴、最小限のプロフィール表示に絞り、admin activeユーザーだけに最小の管理者表示を出す方針へ整理済みである。追加レビューでは `/account` のraw `status` / `active` 表示と `/admin` 非許可画面の内部説明を2名目共有前の小P1として扱い、Preview反映後確認まで完了した。表示名編集、通知設定保存、通知モード、quiet hours、1日の通知上限、forgot password、username変更、avatar設定は後続課題として扱う。Phase 10の1名テストでは、保存できない入力フォームを参加者向けUIに出さない。

2026-06-07時点で、2名目共有前の追加GPT ProレビューではP0/P1実装修正なしの判断である。2名目共有前read-only確認では、admin 1名と通常user 1名のactive member 2名、`remainingSeats=8` を確認した。2名目用の参加者別invite codeは1件発行済みで、`max_uses=1`、active、`use_count=0`、`admin_audit_logs` の `invite_created` を確認済みである。参加者別invite code実値はdocs/repoに書いていない。2名目へのPreview URL / invite code共有実行はまだ行っていない。
...
| `quiz-world-phase-10-first-participant-test-results.md` | Phase 10の信頼できる1名への個別共有開始後の結果。read-only DB確認、admin保護確認、P0/P1/P2、1名継続GO/2名目保留を記録する。 |
| `quiz-world-phase-10-second-participant-prep-results.md` | Phase 10の2名目共有前確認結果。GPT Proレビュー、read-only DB確認、admin保護確認、2名目用invite発行、未共有状態を記録する。 |
...
- Phase 9 Step H完了時点ではPreview URL共有範囲をowner/adminのみに維持した。現在はPhase 10で信頼できる1名への個別共有を開始済みである。10人テスト候補全員への共有はまだ行わない。`v0.10.0-phase9-preview-ready` tagは作成・push済みである
...
- 1名テストのsignup / login / 主要ループ / 不具合報告を継続確認し、結果をdocsに記録する。
- 1名テストの残フィードバックを確認し、P0/P1が増えていないか記録する。
- 2名目用の参加者別invite codeは発行済み。2名目へ拡張する場合は、Preview URLとcodeを個別DMでのみ共有し、共有実行結果をdocsに記録する。
- 2名目への共有実行結果をdocsに記録する。
...
- profileのrank説明を、10人テスト前の既知制約として扱うか最終確認する。
- `/world` の補助指標はPhase 10の1名テストでは非表示に整理済み。平均評価や参加枠解放条件は、実データ集計定義と参加枠増加ルールを正式化する後続課題として扱う。
