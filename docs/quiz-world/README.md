...
- Vercel Deployment Protectionのowner/adminアクセス、または安全なbypass手順を確定する。
...
- 次回のPreview deploymentは、Production Branchが `production-hold` であることを再確認した上で、`preview` branchを `origin/main` の `be62e73` に合わせてpushし、Git連携Preview deployで作り直す。
