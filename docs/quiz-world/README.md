...
| Phase 0の技術初期化詳細 | Next.jsのテンプレート、lint/test runner、UIライブラリを決める。 |
| 初期admin付与方法 | `ADMIN_EMAILS` を使うか、seed/手動SQLで付与するかをPhase 0で決める。 |
| RLS policy test環境 | Supabase localで実行する前提だが、テスト方法を実装準備で具体化する。 |
| `NOTIFICATION_PHASE` の扱い | env切り替えにするか、MVP初期はコード定数にするかをPhase 0で決める。 |
...
Phase 0ではQuiz World専用リポジトリ内のローカルアプリ土台のみを扱う。

## 実装へ進む前の最終チェック

- `quiz-world-pre-implementation-decisions.md` の最終固定内容をChatGPT Proへ共有する。
- 実装開始前に、現在のdocs差分をcommitする。
- Phase 0で作るものと作らないものを確認する。
- 実装開始時もSmart Buzzerのproduction / Stripe / Vercel / Supabase / env / legal page / cleanup / live keyに触れないことを再確認する。
- DB migration SQLとcloud環境作成は、Phase 0の合意後に別タスクとして扱う。
...
- プロジェクト初期化方針を決める。
- Next.js / TypeScript / lint / test runner を決める。
- Supabase localの起動方針を決める。
...
Phase 0の実装準備に進める状態。
ただし、実装開始前にdocs設計フェーズの内容をcommitする。
