...
Phase 3のquiz launch / recipients local実装はSupabase local DB込みで検証済みである。
Supabase / Vercel / Stripe のcloud環境、production deploy、回答、ランキング、admin本実装、Realtime、Web Pushはまだ扱わない。
...
| Phase 4の問題本文返却条件 | `/quiz/[launchId]` で `start_at` 到達後に問題本文・選択肢を返す条件を回答実装前に確定する。 |
...
- Phase 3計画範囲外の本格API実装
- Supabase / Vercel / Stripe のcloud環境作成

Phase 3 local実装段階ではQuiz World専用リポジトリ内の `quiz_launches` / `quiz_recipients` / 最小 `blocks`、関連API、`/home`、`/create` 導線のみを扱う。
...
- Phase 3完了地点は必要に応じてtagで固定する。

## 次の作業候補

### A. Phase 3完了地点の固定

- 必要に応じてPhase 3完了地点のtagを作成する。
- tag作成前に `git status` と最新commitを確認する。

### B. Phase 4計画docs

- Phase 4の回答受付、answers、answer_rank / correct_rank、`/quiz/[launchId]` の問題本文返却条件をdocs化する。

## 現時点の推奨

Phase 3完了地点を固定し、その後にPhase 4回答実装の計画docsへ進む。
