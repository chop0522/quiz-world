...
docs設計フェーズとPhase 0のローカル土台作成は一区切り済みで、現在はPhase 1のsignup/authローカル実装段階である。
Phase 1ではSupabase localを前提に、signup、login、18歳以上確認、規約同意、招待コード、参加枠確認、waitlist誘導を扱う。
Supabase / Vercel / Stripe のcloud環境、production deploy、クイズ作成API、通知、ランキング、admin本実装はまだ扱わない。
...
| Supabase local実行環境 | Docker DesktopまたはDocker互換runtimeを起動してlocal DB検証を行う。 |
...
Phase 1ではQuiz World専用リポジトリ内のlocal signup/auth実装のみを扱う。

## 実装へ進む前の最終チェック

- Phase 1 signup/auth local実装の検証結果をChatGPT Proへ共有する。
- Phase 1実装差分をcommitする前に、Supabase local / npm検証 / README整合を確認する。
...
- cloud環境作成、Stripe、production deployはPhase 1では扱わない。

## 次の作業候補

### A. Phase 1 local実装の検証

- Supabase localを起動する。
- `.env.local` にlocal keyを設定する。
- signup/login/APIの最小動作を確認する。
- `npm run typecheck`、`npm run lint`、`npm run test`、`npm run build` を通す。
- 2026-05-21時点で、Phase 1 signup/auth local実装はDB込み検証済み。

### B. Phase 1実装差分のcommit
...
Phase 1のsignup/auth local実装はcommit前レビューに進める状態。commit前にgit statusと差分範囲を確認する。
