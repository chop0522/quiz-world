...
- `/profile` P1整理後の表示を確認し、通常ユーザーにraw `role` / `status` や保存できないフォームが出ないことを確認する。
