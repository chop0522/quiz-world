...
現在はPhase 6のrank_events / ranking計画中である。
Supabase / Vercel / Stripe のcloud環境、production deploy、複雑なランキング、admin本実装、Realtime、Web Pushはまだ扱わない。
...
- Phase 6計画範囲外の本格API実装
- Supabase / Vercel / Stripe のcloud環境作成

Phase 6実装へ進む場合も、対象は rank_events、score更新、rank更新に限定する。
...
Phase 6では、回答結果とクイズ評価を `rank_events` として記録し、MVP向けのシンプルな回答者/出題者スコア更新を扱う。
...
- `quiz-world-phase-6-rank-events-plan.md` をChatGPT Proに渡してレビューする。
- rank event発火タイミング、rank閾値、score下限、backfill有無を決める。

### B. Phase 6 local実装

- Supabase local前提で rank_events、score更新、rank更新を実装する。
- Web Push、Realtime、admin本実装、cloud環境はまだ作らない。

## 現時点の推奨

Phase 6計画docsをレビューし、実装前の小さな固定事項を決めてからPhase 6 local実装へ進む。
