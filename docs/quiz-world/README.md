...
- 最終GO/NO-GO判断はStep DでVercel project作成済み。Step D follow-upでGitHub repo接続済み。Step EはProduction Branchを `production-hold` へ変更後にVercel Preview env設定済み。Step F Preview deploy前GO/NO-GOはGO候補。ただしStep F開始時にIgnored Build Step変更が必要。Production envとPreview deployは未実行。
...
- Step F Preview deploy前GO/NO-GOではGO候補。Ignored Build Stepは現在 `exit 0` のため、Step F開始時に `preview` branchだけbuildを許可する条件式へ変更する
- `NEXT_PUBLIC_APP_URL` はPreview URL未確定のため未設定。現状のapp codeでは参照されておらず、未設定でもlocal `vercel build` は成功済み。Preview URL発行後に必要なら設定する
