...
現在は人間操作による最終UI rehearsalの計画段階である。
...
### A. Phase 8 local smoke results / manual UI rehearsal planレビュー

- Phase 8 resultsをレビューする。
- manual UI rehearsal planをレビューする。
- 10人テスト前に残る課題を確認する。

### B. 人間操作での最終UI rehearsal実行

- ブラウザ自動操作ではなく、人間操作で `/signup` から `/admin` までの最終UI確認を行う。
- 実行時もcloud環境やproduction deployには進まない。
...
Phase 1〜7 local実装は完了・push・tag済み。Phase 8 10-user local smoke / ops rehearsalもSupabase localのみで実行済み、89チェックpass、DB reset済み。次はPhase 8 resultsとmanual UI rehearsal planをレビューし、人間操作での最終UI rehearsalをSupabase localだけで実行する。その後、Phase 9 Preview環境検討へ進むか判断する。
