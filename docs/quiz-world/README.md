...
現在はPhase 7 admin / moderation の実装計画中である。
...
Phase 7では、10人テストを安全に運用するための簡易admin / moderation機能を計画する。
...
### A. Phase 7 plan review

- Phase 7 admin / moderation planをChatGPT Proへ共有する。
- admin API、audit log、question/user停止、invite/waitlist管理の範囲を確認する。

### B. Phase 7実装前固定事項

- `review_required` questionの通常ユーザー扱いを固定する。
- user停止時に `world_members.status` も同時に `suspended` にするか決める。
- report status更新APIを追加するか、question moderation APIへ含めるか決める。
- invite codeをadmin入力にするか、server生成にするか決める。

## 現時点の推奨

Phase 6 local実装は完了・push・tag済み。次はPhase 7 admin / moderation planをレビューし、実装前の小さな固定事項を決める。
