...
現在はPhase 8 10-user local smoke / ops rehearsal計画中である。
...
### A. Phase 8 10-user local smoke / ops rehearsal計画レビュー

- Phase 8 planをレビューする。
- 10人テスト前のlocal smoke / ops rehearsal手順として不足がないか確認する。

### B. Phase 8 local smoke / ops rehearsal実行準備

- Supabase localでのテストユーザー、招待コード、確認用question、admin操作手順を準備する。
...
Phase 1〜7 local実装は完了・push・tag済み。Phase 8 10-user local smoke / ops rehearsal planも作成済み。次はPhase 8計画をレビューし、問題なければSupabase localだけで運用リハーサルを実行する。
