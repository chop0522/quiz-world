...
- 現在の `supabase/seed.sql` はlocal用 `SEASON0-TEST-001` を投入するため、Preview reset前に `SEASON0-PREVIEW-001` の再投入手順を決める必要がある
