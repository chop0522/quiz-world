...
- category_note は `その他` を選んだ場合だけ許可し、出題者本人とadminのみ参照できる。
- 回答者向けAPIや結果画面には category_note を出さない。
...
| category_note | text | その他を選んだ場合の補足テキスト。nullable。出題者本人とadminのみ閲覧可。 |
...
回答者には category が `その他` であることだけを表示し、MVPの結果画面では category_note を表示しない。
...
管理操作と admin_audit_logs 記録は同一transaction扱いにし、ログを残せない場合は管理操作全体を失敗扱いにする。
