...
今回、実装開始前に残っていた利用規約・プライバシーポリシー草案、新規Supabase/Vercel作成タイミング、admin audit transaction、Phase 1.5 Realtime範囲、カテゴリ補足テキストの表示範囲も固定した。
...
| カテゴリ補足 | `category_note` は出題者本人とadminのみ閲覧可。回答者と結果画面には出さない。 |
...
| audit transaction | admin操作とadmin_audit_logs記録は同一transaction扱い。ログ記録失敗時は操作全体を失敗扱い。 |
| Phase 1.5 Realtime | `/home` の本人宛 quiz_recipients 新着だけ対象。結果画面RealtimeはMVP後回し。 |
...
admin操作とadmin_audit_logs記録は同一transaction扱いにし、ログが残せない場合は操作全体を失敗扱いにする。
...
- Phase 1.5では `/home` の本人宛 quiz_recipients 新着だけSupabase Realtime化を検討する。
- 結果画面のRealtime更新はMVP後回しにする。
...
| 追加API | 7本を正式追加。 |
| 利用規約・プライバシーポリシー草案 | 実装開始前に草案ページを用意し、10人テスト前に専門家確認。 |
| 新規Supabase環境 | Phase 0〜3はSupabase local推奨。Productionは10人テスト直前。 |
| 新規Vercel環境 | Phase 0では不要。Previewはlocal主要機能後、Productionは10人テスト直前。 |
| admin_audit_logs失敗時 | 同一transaction扱い。ログ記録失敗時は操作全体を失敗扱い。 |
| Phase 1.5 Realtime範囲 | `/home` の本人宛 quiz_recipients 新着のみ。 |
| カテゴリ補足表示範囲 | 出題者本人とadminのみ。結果画面には出さない。 |

## まだ実装前に確認すること

1. docs設計フェーズの差分をcommitしてからPhase 0へ進む。
2. Phase 0の技術初期化詳細を決める。
3. 初期admin付与方法を決める。
4. RLS policy testの実行方法を具体化する。
