...
| Framework Preset明示後のStep G smoke本体 | 未実行。`/` と `/api/world` の到達確認、MVP主要ループ確認はまだ行っていない |
