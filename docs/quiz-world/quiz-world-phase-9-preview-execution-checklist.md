...
- 最終GO/NO-GO判断: Step DでVercel project作成済み。Step D follow-upでGitHub repo接続済み。Step E再開前安全作業で `production-hold` branch作成とProduction Branch変更は完了。Step EでVercel Preview env設定済み。Step F Preview deployはReady。Framework Preset明示後のStep G Preview smoke本体はpass。Step HはGO候補。ただしPreview DB cleanup / reset方針の決定が必要
...
- この設定変更時点では、Framework Preset明示後のStep G smoke本体はまだ再実行していなかった
- secret実値、Supabase key、Vercel token、bypass secret、初期admin email実値はdocs/repoに記録していない

Framework Preset明示後のStep G再実行用Git連携Preview deployment作成結果:

- 実行日時: `2026-05-26 20:59 JST`
- `preview` branchを `origin/main` の最新commit `7d63505` に合わせてpushした
- 空commitは不要だったため作成していない
...
- deployment source: Git連携
- deployment branch / commit: `preview` / `7d63505`
- deployment environment: Preview
- deployment status: Ready
- Build logでNext.js routesが出力され、`/`、`/signup`、`/login`、`/home`、`/create`、`/quiz/[launchId]`、`/result/[launchId]`、`/admin`、`/api/world` などが含まれることを確認した
- Production deploymentは追加作成されていない。既存Production deployment 2件のみ
- Production env / Production custom domainは未設定のまま
- このdeployment作成時点では、Step G smoke本体はまだ実行していなかった。その後、下記の入口確認を実行した
- secret実値、Supabase key、Vercel token、bypass secret、初期admin email実値はbuild log / docs / repoに記録していない

Framework Preset明示後のStep G入口確認結果:

- 実行日時: `2026-05-26 21:18 JST`
- 通常アクセスでは `/` と `/api/world` が401 Deployment Protection
- Vercel CLIのprotection bypass経由では `/` がQuiz WorldのHTMLを返し、`<title>通知型早押しクイズワールド</title>` を確認した
- Vercel CLIのprotection bypass経由では `/api/world` が `{"ok":true}` と初期world `クイズワールド`、`memberLimit: 10`、`remainingSeats: 10` を返した
- Shareable Linkは発行していない
- この時点ではMVP主要ループ本体はまだ実行していなかった。その後、同じdeploymentでStep G smoke本体を実行しpassした
- Production deploymentは追加作成されていない
- Production env / Production custom domainは未設定のまま
- secret実値、Supabase key、Vercel token、bypass secret、初期admin email実値はdocs/repoに記録していない
- Step G入口確認はGO候補。この後のStep G smoke本体もpass済み
...
Step EでVercel Preview envは設定済みである。Step FでPreview deployは実行済みである。Step G smokeはFramework Preset明示後の新Preview deploymentでpass済みである。現時点でも、以下はまだ実行しない。
...
Step AからStep Cまでは以下を満たして実行済みである。Step DではVercel Preview project作成を行い、Step D follow-upでGitHub repo接続まで完了した。Step EでVercel Preview env設定まで完了した。Step FでPreview deployはReadyになった。Step Gは一度Deployment Protection / bypass未解決によりNO-GOだったが、Framework PresetをNext.jsに明示した新しいGit連携Preview deploymentで入口確認とMVP主要ループ本体をpassした。Step HへはGO候補とする。ただし、Preview DB cleanup / reset方針を決めるまではowner/admin以外へPreview URLを共有しない。Production deployへ進む場合は、別途GO/NO-GO判断を行う。
...
| Step G NO-GO原因修正 | Vercel Project SettingsでFramework PresetをNext.jsに明示。Root Directory / Build Command / Output Directory / Install Commandはdefaultのまま |
| Framework Preset明示後のStep G再実行用Git連携Preview deployment | `https://quiz-world-preview-ri8igtw45-chop0522s-projects.vercel.app` / `dpl_6YhA6LJudsrnBEbJ4UPdgGPwmUkx` / Git連携 / `preview` / `7d63505` / Ready |
| Framework Preset明示後のStep G入口確認 | 通常アクセスは `/` と `/api/world` が401 Deployment Protection。Vercel CLIのprotection bypass経由では `/` がQuiz WorldのHTML、`/api/world` が初期world JSONを返した |
| Framework Preset明示後のStep G smoke本体 | pass。signup、admin role、login、question作成、launch、/home、start_at前後、answer、result、rating、report、rank_events、admin確認、admin_audit_logsまで49項目を確認 |
| Preview DB cleanup / reset | 必要。Step G smokeでtest users、question、launch、answers、rating、report、rank_events、admin_audit_logs、追加invite、suspended userが作成された |
| 初期admin email | 決定済み。docsには実値を書かない。Vercel Preview envの `ADMIN_EMAILS` にのみ設定 |
| Preview invite code | `SEASON0-PREVIEW-001` |
| Preview共有先 | owner/adminのみから開始 |
| cleanup担当 | 自分 |
| GO / NO-GO判断 | Step G Preview smoke本体はpass。Step HはGO候補。ただしPreview DB cleanup / reset方針を決めるまで共有範囲はowner/adminのみ |
