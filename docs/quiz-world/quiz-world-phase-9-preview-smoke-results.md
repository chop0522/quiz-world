...
このdeployment作成時点では、次回Step G smokeで最初にDeployment Protection通過後の `/` と `/api/world` 到達を確認し、到達できる場合のみMVP主要ループへ進む方針としていた。その後、下記の入口確認を実行した。

## Step G入口確認結果（Framework Preset明示後）

2026-05-26 21:18 JSTに、Framework PresetをNext.jsへ明示した後の新しいGit連携Preview deploymentで、Step G smoke再実行の入口確認だけを実行した。

この作業では、MVP主要ループ本体、Production deploy、Production env設定、Production custom domain設定、Stripe、Web Push、Realtimeは実行していない。Smart Buzzerにも触っていない。
...
| 対象 | 通常アクセス | protection bypass経由 | 判断 |
| --- | --- | --- | --- |
| `/` | 401 Deployment Protection | Quiz WorldのHTMLを返却。`<title>通知型早押しクイズワールド</title>` を確認 | 入口到達OK |
| `/api/world` | 401 Deployment Protection | `{"ok":true}` と初期world `クイズワールド`、`memberLimit: 10`、`remainingSeats: 10` を返却 | 入口到達OK |

通常アクセスの401はVercel Deployment Protectionとして想定内である。Vercel CLIのprotection bypass経由では `/` と `/api/world` に到達できたため、Framework Preset明示前に発生していた404はこのdeploymentでは再現していない。

Shareable Linkは発行していない。bypass secret、Shareable Link実値、Supabase key、Vercel token、初期admin email実値はdocs/repoに記録していない。

### 判断

Step G入口確認は **GO候補**。

ただし、この時点では入口確認だけであり、signup、login、question作成、launch、answer、result、rating、report、rank_events、admin操作までのMVP主要ループ本体はまだ実行していなかった。そのため、この時点ではStep G smoke全体はまだpass扱いにしていなかった。`v0.10.0-phase9-preview-ready` tagもまだ作っていない。

次は同じPreview deploymentで、Vercel Deployment Protectionを通過する方法を使い、MVP主要ループ本体のStep G smokeを実行する。

## Step G Preview smoke本体結果（Framework Preset明示後）

2026-05-26 21:31-21:41 JSTに、Framework PresetをNext.jsへ明示した後のGit連携Preview deploymentで、Step G Preview smoke本体を実行した。確認はVercel Deployment Protectionを通過するため、Vercel CLIのprotection bypass経由で行った。

この作業では、Production deploy、Production env設定、Production custom domain設定、Stripe、Web Push、Realtimeは実行していない。Smart Buzzerにも触っていない。
...
| 項目 | 結果 |
| --- | --- |
| 画面到達 | `/`、`/signup`、`/login`、`/home`、`/create`、`/profile`、`/admin`、`/world`、`/invite`、`/legal/terms`、`/legal/privacy`、`/quiz/[launchId]`、`/result/[launchId]` に到達 |
| `/api/world` | 初期world `クイズワールド`、`memberLimit: 10` を返却 |
| invite | Preview invite code `SEASON0-PREVIEW-001` でvalidate / signup成功。local用 `SEASON0-TEST-001` はPreview smokeで使っていない |
| signup / admin | 初期admin email対象ユーザーは `profiles.role = admin`、通常ユーザーは `user` として作成 |
| login / profile | signup後のsessionで `/api/profile` を取得 |
| question作成 | activeな四択questionを作成し、自分のquestion一覧で確認 |
| launch | active questionをlaunchし、recipientに配信 |
| `/home` / quiz一覧 | recipient本人宛のlaunchを取得 |
| start_at前 | 問題本文・選択肢が返らないことを確認 |
| start_at後 | 問題本文・選択肢が返ることを確認 |
| answer | 正解回答と不正解回答を作成。正解は `answer_rank = 1`、`correct_rank = 1`、不正解は `correct_rank = null` |
| result | recipient / author双方でresultを取得。全回答者一覧、未回答者一覧、`correctChoiceId` のresult表示を確認 |
| rating | recipientがratingを作成し、出題者向けrank eventが作られることを確認 |
| report | recipientがreportを作成し、status `open` を確認 |
| rank_events | 回答者score/rank events、出題者score/rank eventsを `/api/profile` で確認 |
| admin reports / questions / users | admin APIでreport、question、user一覧とreport詳細を確認 |
| admin waitlist / invites | admin APIでwaitlist / invite一覧を確認 |
| admin操作 | report status更新、question `review_required`、user suspend、invite作成を実行 |
| admin_audit_logs | `report_reviewed`、`question_review_required`、`user_suspended`、`invite_created` のaudit logを確認 |
| user suspend | `profiles.status = suspended` と `world_members.status = suspended` を確認 |

自動確認は49項目pass。途中で検証スクリプト側のjq assertion引数指定ミスが2件あったが、APIレスポンス自体は期待形で、修正後の確認はpassしている。
...
| pass / fail | pass |
| P0 | なし |
| P1 | Preview DB cleanup / resetが必要。smokeでtest users、question、launch、answers、rating、report、rank_events、admin_audit_logs、追加invite、suspended userが作成された |
| P2 | `NEXT_PUBLIC_APP_URL` は未設定のまま。今回確認した画面/APIではruntime blockerにならなかったが、共有URLやabsolute URLが必要な機能を入れる前にPreview URLで設定するか再検討する |
| Preview DB cleanup要否 | 必要。10人テスト候補へ共有する前にPreview DBをresetまたは検証データをcleanupする |
| Production deploy | 追加作成なし。既存の記録済みProduction deployment 2件のみ |
| Production env / custom domain | 未設定のまま |
| Smart Buzzer混入 | `/api/world`、Preview invite、admin/API確認の範囲でSmart Buzzer由来データは見つからない |
| secret hygiene | Supabase key、service role key、DB password、初期admin email実値、Vercel token、bypass secretはdocs/repoに記録していない |

### Step H判断

Step G Preview smoke本体は **pass**。

Step Hへは **GO候補** とする。ただし、Step Hへ進む前に、Preview DB cleanup / reset方針を決め、owner/admin以外へPreview URLを共有しない状態を維持する。`v0.10.0-phase9-preview-ready` tagは、Step G結果のcommit / pushと必要なcleanup判断後に作成する。

## 10人テスト前に残る課題

| 優先度 | 課題 | 方針 |
| --- | --- | --- |
| 解消済み | Preview URLのDeployment Protectionにより通常smoke不可 | Vercel CLIのprotection bypass経由で `/`、`/api/world`、MVP主要APIを確認済み。実値はdocsに書いていない |
| 解消済み | 新しいGit連携Preview deploymentでもVercel CLI bypassが `404_NOT_FOUND` になる | Framework PresetをNext.jsに明示した新deploymentでは404は再現せず、Step G smoke本体までpass |
| 解消済み | Framework Presetが未指定 | Vercel Project SettingsでFramework PresetをNext.jsに明示済み。Git連携Preview deployment作り直しとStep G smoke本体も完了 |
| 解消済み | Framework Preset明示後のPreview deployment未確認 | 新しいGit連携Preview deploymentを `preview` / `7d63505` で作成済み。statusはReady |
| 解消済み | Framework Preset明示後のPreview deployment到達未確認 | Vercel CLIのprotection bypass経由で `/` と `/api/world` への到達を確認済み |
| 解消済み | CLI deploymentのartifact / routingが不審 | Git連携Preview deploymentへ切り替え、さらにFramework Preset明示後のdeploymentでStep G smoke本体をpass |
| 解消済み | `preview` branchがmainより古い | `preview` branchを最新の `origin/main` へ合わせてpush済み |
| P1 | Preview DB cleanup / reset | Step G smoke本体で検証データを作成済み。10人テスト候補へ共有する前にresetまたはcleanupする |
| P2 | `NEXT_PUBLIC_APP_URL` 未設定 | 今回確認した画面/APIではruntime blockerなし。共有URLやabsolute URLが必要な機能を入れる前にPreview URLで設定するか再検討する |

## Step Hへ進めるか

**GO候補（Step G Preview smoke本体はpass）**。

ただし、Step Hへ進む前にPreview DB cleanup / reset方針を決める。Preview URLの共有範囲は引き続きowner/adminのみとし、10人テスト候補全員への共有はまだ行わない。

推奨する次アクション:

1. Step G結果をcommit / pushする。
2. Preview DB cleanup / reset方針を決める。
3. 必要なら `v0.10.0-phase9-preview-ready` tag作成前の最終GO/NO-GO確認を行う。
