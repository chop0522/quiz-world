...
import { validateAdminQuestionModerationPayload } from "@/lib/phase7-validation";
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const parsed = validateAdminQuestionModerationPayload(rawBody);
    ...
    return NextResponse.json(
      {
        ok: false,
        errors: [
          error instanceof Error ? error.message : "question moderationに失敗しました。"
        ]
      },
      { status: 500 }
    );
